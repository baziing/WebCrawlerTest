...
def main():
    ...
    browser.get('https://www.taptap.com/top/new')
    time.sleep(3)
    refresh(browser)
    ...
    for day in gameByDay:
        ...
        df.loc[i,'评分']=day.find('.tap-row-card__contents.flex-1.x-start-y-center .app-rating__number.font-bold.rate-number-font').text()
...
main()
# print(df)
df.to_excel('tmp/新品.xlsx',index=False)